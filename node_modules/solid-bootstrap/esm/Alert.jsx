...
const Alert = (uncontrolledProps) => {
    ...
    const alert = () => (<div role="alert" {...(!Transition ? props : {})} class={classNames(local.class, local.className, prefix, local.variant && `${prefix}-${local.variant}`, local.dismissible && `${prefix}-dismissible`)}>
      {local.dismissible && (<CloseButton onClick={handleClose} aria-label={local.closeLabel} variant={local.closeVariant}/>)}
      {local.children}
    </div>);
    return (<Show when={!!Transition} fallback={local.show ? alert : null}>
