...
const ListGroupItem = (p) => {
    ...
    const disabledProps = () => local.disabled && props.tabIndex === undefined
        ...
        : {};
    return (<Dynamic component={local.as || (local.action ? (props.href ? "a" : "button") : "div")} {...props} {...navItemProps} {...disabledProps()} onClick={handleClick()} class={classNames(local.class, local.className, bsPrefix, meta.isActive && "active", local.disabled && "disabled", local.variant && `${bsPrefix}-${local.variant}`, local.action && `${bsPrefix}-action`)}/>);
};
